#!/usr/bin/env python3
"""Build the compact history fixture from one timestamped two-sensor window.
...
segments; it is never interpolated or converted to zero usage. The source
directory is intentionally untracked because it contains the original capture;
the compact generated profile is the firmware input committed to this project.
"""
...
def main():
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=ROOT / "demo-source",
        help="captured demo-source directory (default: esp32-viewe/demo-source)",
    )
    args = parser.parse_args()
    content, stats = build(args.source.resolve())
    if args.check:
