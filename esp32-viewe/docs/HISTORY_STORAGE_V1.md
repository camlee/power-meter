...
| --- | --- | --- |
| `Real` (`r`) | ESP32 ADC and Arduino UART | Actual device observations. |
...
- Settings -> History `Real | Demo` filters the catalog and local Usage views;
- the filter does not install, remove, copy, or reclassify files;
- the filter defaults to Real after boot and every Demo view is visibly labeled;
- browser queries name their dataset explicitly and default to Real when an
  older caller omits it.

The UI should show both `Recording: <dataset/source>` and `Viewing: <dataset>`
when they differ. A Demo source continues recording into Demo even while the
user views Real history, and those files appear as soon as Demo is selected.
...
the fixture anchor is pinned once to the newest complete local-day window that
does not overlap any resolved recorded Demo interval. On a fresh device this is
normally the complete days immediately preceding today. It then remains fixed.
...
Every catalog/query call takes an explicit dataset. Only the selected directory
is cataloged, keeping normal work bounded even though both datasets coexist.
...
The browser API accepts `dataset=real|demo`. Omitted dataset remains `real` for
safe compatibility with the already completed single-dataset web app. Catalog
and query responses include the selected dataset explicitly.
...
Candidate V1 is a design ready for implementation, not current behavior. It
must pass [HISTORY_TEST_PLAN.md](HISTORY_TEST_PLAN.md) before being treated as
production durable. The implementation may freely remove alpha V3 data from the
